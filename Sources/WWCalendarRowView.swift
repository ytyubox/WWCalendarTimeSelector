...
internal class WWCalendarRowView: UIView {
    ...
    internal override func draw(_ rect: CGRect) {
        ...
        if detail.type == .month {
            ...
            let attributes = [NSAttributedString.Key.font: monthFont, NSAttributedString.Key.foregroundColor: monthFontColor, NSAttributedString.Key.paragraphStyle: paragraph].compactMapValues{$0}
            let str = NSAttributedString(string: monthName, attributes: attributes)
            ...
            for (index, element) in days.enumerated() {
                let attributes = [NSAttributedString.Key.font: dayFont, NSAttributedString.Key.foregroundColor: dayFontColor, NSAttributedString.Key.paragraphStyle: paragraph].compactMapValues{$0}
                let str = NSAttributedString(string: element, attributes: attributes)
                ...
                if date.weekday == i {
                    ...
                    if comparisonDates.contains(date) {
                        ...
                        if multipleSelectionEnabled {
                            let attributes = [NSAttributedString.Key.font: dateTodayFontHighlight, NSAttributedString.Key.paragraphStyle: paragraph].compactMapValues{$0}
                            let testStringSize = NSAttributedString(string: "00", attributes: attributes).size()
...
//                                testStringSize = NSAttributedString(string: "00", attributes: [NSAttributedString.Key.font: datePastFontHighlight, NSAttributedString.Key.paragraphStyle: paragraph]).size()
    ...
    internal func flashDate(_ date: Date) -> Bool {
        let detail = delegate.WWCalendarRowGetDetails(tag)
